...
<>
        ...
        {(portfolioTotals || sp500Performance) && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8"> {/* Adjusted grid columns */}
            {/* Market Value */}
        ...
        {/* --- Holdings Details (Replaces Main Table) --- */}
        <div className="mt-8 space-y-8"> {/* Added space-y for separation */}
          ...
          {sortedSymbolList.length === 0 && (
             <div className="text-center py-6 text-gray-500">No holdings found.</div>
          )}
          {sortedSymbolList.map((item) => {
            // Use the calculated openPosition from the state
            const position = item.openPosition;
            // const isCurrentlyOpen = item.currentQuantity !== 0; // Still potentially useful for display logic inside
            ...
            return (
              // Render the "expanded" content directly for each symbol
              <div key={symbol} className="border-b border-gray-200 dark:border-gray-600 pb-6 last:border-b-0 last:pb-0 mb-6"> {/* Reduced padding and border weight */}
                  {/* Display the sections previously inside the expanded row */}
                  <div className="flex flex-col md:flex-row md:space-x-6 md:space-y-0 items-stretch"> {/* Changed to items-stretch for equal height */}

                    {/* Flex Item 1: Position Details/Status Card */}
                    <div className="flex-shrink-0 w-full md:w-1/3"> {/* Keep width */}
                      {/* === START Conditional Rendering === */}
                      {position ? (
                        // Display Open Position Details calculated earlier
                        <Card className="h-full shadow-sm border border-gray-200 dark:border-gray-700"> {/* Added h-full */}
                          <CardContent className="p-4">
                            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 gap-x-4 gap-y-3">
                            <DetailItem label="Symbol" value={symbol} />
                        ...
                        <Card className="h-full shadow-sm border border-gray-200 dark:border-gray-700"> {/* Added h-full */}
                          <CardHeader className="bg-gray-100 dark:bg-gray-700 py-3 px-4 border-b border-gray-200 dark:border-gray-600">
                            <CardTitle className="text-lg font-medium">{symbol}: Position Closed</CardTitle>
                          </CardHeader>
                          <CardContent className="p-4">
                            {item.realizedPL !== 0 && (
                    ...
                    {/* Flex Item 2: Tabs Section */}
                    {hasAdditionalData && (
                      <div className="flex-grow w-full md:w-2/3 mt-4 md:mt-0"> {/* Keep width */}
                        <Card className="h-full shadow-sm border border-gray-200 dark:border-gray-700"> {/* Added single card wrapper */}
                          <CardContent className="p-4 h-full"> {/* Added h-full */}
                            <Tabs defaultValue={hasDisplayableTrades ? "history" : (hasClosedLots ? "closed" : undefined)} className="h-full">
                              <TabsList className="w-full bg-gray-200 dark:bg-gray-700 p-1 rounded-md">
                                ...
                                {hasClosedLots && (
                                  <TabsTrigger value="closed" className="flex-1 py-2 data-[state=active]:bg-white data-[state=active]:dark:bg-gray-950 data-[state=active]:shadow-sm data-[state=active]:font-semibold">
          ...
          {/* Removed closing table structure */}
        </div>
        </>
  );
};
 ...
 ({ label, value, format, colored, className, tooltip }) => {
    ...
    if (value === 'N/A') {
        ...
        if (colored) {
             ...
             // Simplified the logic: we already know 'value' is a number here.
            const numberValue = isFinite(value) ? value : (value > 0 ? 1 : (value < 0 ? -1 : 0));
